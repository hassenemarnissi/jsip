...
public class Test2S implements SipListener {
    ...
    private static Logger logger = Logger.getLogger(Test2S.class);
    ...
    private synchronized void runStateMachine() {
      ...
      while (runAgain)
      {
        /*
         * C             S  State  0 = start
         * |--INVITE-1-->|    0 -> 1 = recieved INVITE-1
         * |<---100-1----|    1 -> 2 = sent 100-1
         * |<---200-1----|    2 -> 3 = sent 200-1
         * |----ACK-1--->|    3 -> 4 = received ACK-1
         * |     ...     |             short period passes...
         * |--INVITE-2-->|    4 -> 5 = received INVITE-2
         * |<---401-2----|    5 -> 6 = sent 401 unauth for 2
         * |<-INVITE-10--|    6 -> 7 = sent reINVITE 10 from peer
         * |----200-10-->|    7 -> 8 = received 200-10
         * |----ACK-2--->|    8 -> 9 = received ACK-2 (for 401/INVITE 2)
         * |<---ACK-10---|    9 ->10 = sent ACK-10
         * 
         * In the failure case, ACK-10 is dropped and C continues:
         * |----200-10-->|           - Fail (too many OKs) // Though note this app probably won't see the extra OK - it's handled in the stack.
         * |<---ACK-10---|
         * |     ...     |
         * |----200-10-->|
         * |<---ACK-10---|
        ...
        switch (state)
        {
          ...
          case 6:
          {
            ///*sendInvite(10L); // should use*/ sendReInvite(10L);
          ...
          case 7:
          {
            //verifyLastResponse("INVITE", 10L, 200);
          ...
          case 8:
          {
            //verifyLastRequest("ACK", 2L); // we shouldn't actually send ACK after 401 so this is wrong
          }
          break;
